...
dts_solucion = [] #guardo los tiempos en que se invierte A por corrida
dts_ensamblaje = [] #guardo los tiempos en que se ensambla el sistema por corrida

txt = open("timing_INV_Matriz_Dispersa_double.txt","w")
...
for corrida in corridas:
    for N in Ns:
        ...
        A = sp.csc_matrix(laplaciana(N))
        
        t2 = perf_counter()
        
        x = lin.inv(A) #Invierto A
        ...
        txt.write(f"N = {N}; Tiempo inversión = {dt_solucion} seg.; Tiempo ensamblaje = {dt_ensamblaje} seg.\n" )
        
        print ("Corrida =", corrida+1)
        print ("N =", N)
        print (f"Tiempo ensamblaje: {dt_ensamblaje} seg.")
        print (f"Tiempo inversión: {dt_solucion} seg.")
        print ("----------------------------------------------------")
...
for i in range(len(corridas)):
    ...
    plt.grid()
    plt.ylabel("Tiempo de inversión")
    plt.title("Desempeño de INV para Matrices Dispersas")
