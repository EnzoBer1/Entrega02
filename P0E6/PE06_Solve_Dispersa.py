...
txt = open("timing_SOLVE_Matriz_Dispersa_double.txt","w")
...
for corrida in corridas:
    for N in Ns:
        ...
        A = sp.csr_matrix(laplaciana(N))
        b = np.ones(N)
        
        t2 = perf_counter()
        
        x = lin.spsolve(A,b) #Soluciono el sistema A*x = b
        ...
        txt.write(f"Corrida = {corrida}; N = {N}; Tiempo solución = {dt_solucion} seg.; Tiempo ensamblaje = {dt_ensamblaje} seg.\n" )
...
for i in range(len(corridas)):
    ...
    plt.title("Desempeño SOLVE para Matrices Dispersas")
